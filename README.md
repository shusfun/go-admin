...
首次部署无需手动编辑配置文件 —— 通过内置的 **Setup Wizard（安装向导）** 即可在浏览器中完成数据库连接配置和管理员账号创建。
...
后续开发流程统一通过 `repo` CLI 执行，推荐从仓库根 `pnpm` 入口直接触发，无需单独构建 CLI。
...
- If project Docker infra is already running, it reuses the published development ports
- If Homebrew PostgreSQL is available, it can reuse `5432`
...
2. **管理员账号** — 设置管理员用户名、密码、邮箱（可选）、手机号（可选）
...
# 停止应用容器
pnpm repo:docker:down
```

容器首次启动时同样会进入 Setup Wizard 模式，通过浏览器完成安装即可。

`repo` 与 `docker compose` 默认都读取仓库名推导出的项目名前缀，当前仓库默认值为 `go-admin`。如需覆盖，可以在命令前传入 `--project-prefix`。
